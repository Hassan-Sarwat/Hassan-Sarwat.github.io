...
I was working on a chatbot with friends and when we considered deploying our own fine-tuned models I became accutely aware of how expensive maintaining it would be, not just that, when building a chatbot something like latency is very crucial to user experience. Given that there are already tens of thousands of AI applications and services, we knew that in order to survive our responses needed to be faster than your response time to someone that doesn't like you back. While the problem might seem solvable by throwing more GPUs at it, we didn't want to burn through our limited funds then hope that someone will give us more money that we can then burn through as well.
...
Therefore the expected per token in speculative decoding is the cost per step/expected tokens.  
...
For this project I wanted to apply speculative decoding as a learning exercise, and also experiment a bit. I've been reading up on reasoning models and realized that they are trained on Chain of Thoughts, which can sometimes ramble on a bit too much. 
...
So now that we have our problem, what are we going to do? In the rest of this blog I will explain the design of the experiment and also spoilers for the rest of the series. If it's a link know that it's ready to be viewed and a blog post
